...
const UbicacionMaterial = () => {
  ...
  const columns = [
    { field: 'id_UbicacionMaterial', header: 'Clave' },
  ...
  useEffect(() => {
    setfiltroubicacionMaterial([...ubicacionMaterialList].sort((a, b) => a.id_UbicacionMaterial - b.id_UbicacionMaterial));
  ...
  return (
    ...
    <Toast ref={toast} />
      ...
      <div className="mx-8 mb-4">
        ...
        <DataTable value={filtroubicacionMaterial.length ? filtroubicacionMaterial :ubicacionMaterialList} editMode='cell' size='small' tableStyle={{ minWidth: '50rem' }}>
          {columns.map(({ field, header }) => {
              return <Column sortable={editando === false} key={field} field={field} header={header} style={{ width: '25%' }} editor={field === 'nombre_UbicacionMaterial' ? (options) => cellEditor(options): null} onCellEditComplete={onCellEditComplete}/>;
          })}
        </DataTable>
      </Panel>              
    </>
...
export default UbicacionMaterial